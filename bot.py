...
# FIXED: bare except in load_vip, check_daily_limit returns 0 for VIP remaining,

# analyze_coin uses asyncio.gather correctly, SentimentAgent gets price_data reused
...
# FIX 1: bare except replaced with proper exception handling
...
# FIX 2: VIP remaining was returning 0 (looks like “0 analyses left”), now returns FREE_ANALYSES_PER_DAY

# FIX 3: when limit exceeded, remaining was negative — now always returns 0

def check_daily_limit(user_id):
if is_vip(user_id):
return True, FREE_ANALYSES_PER_DAY  # VIP shows max, not 0
today = date.today().isoformat()
usage = user_daily_usage.get(user_id, {“date”: “”, “count”: 0})
if usage[“date”] != today:
usage = {“date”: today, “count”: 0}
user_daily_usage[user_id] = usage
remaining = max(0, FREE_ANALYSES_PER_DAY - usage[“count”])
...
try:
    # FIX 4: run PriceAgent and SentimentAgent in parallel with asyncio.gather
    # SentimentAgent.analyze now accepts price_data= to avoid duplicate fetch
    price_agent = PriceAgent(client)
    sentiment_agent = SentimentAgent(client)

    price_data, sentiment_data = await asyncio.gather(
        price_agent.analyze(coin, coin_id),
        sentiment_agent.analyze(coin, coin_id),
    )
...
async def global_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
query = update.callback_query
await query.answer()
...
async def cmd_listvip(update: Update, context: ContextTypes.DEFAULT_TYPE):
user_id = update.effective_user.id
if user_id not in ADMIN_IDS:
await update.message.reply_text(“Нет доступа.”)
return
...
logger.info(“Crypto AI Agents Bot v4 started”)
