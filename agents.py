...
Ispravleniya otnositelno iskhodnogo TZ:

1. AsyncAnthropic vmesto sinhronnogo Anthropic (realnyy parallelizm)
1. Obrabotka oshibok Claude API
1. Ekranirovaniye Markdown-simvolov dlya Telegram
1. Pometka demo-dannye v vyvode (poka net realnogo API)
1. Rate limiting gotovnost
   “””
...
# Konfiguratsiya cherez env peremennye

MODEL = os.getenv(“CLAUDE_MODEL”, “claude-sonnet-4-20250514”)
MAX_TOKENS = int(os.getenv(“CLAUDE_MAX_TOKENS”, “500”))

# Taymaut dlya Claude API zaprosov (sekundy)

API_TIMEOUT = int(os.getenv(“API_TIMEOUT”, “45”))

def escape_markdown(text: str) -> str:
“””
Ekranirovaniye spetssimvolov Markdown V1 dlya Telegram.
Claude mozhet vernut lyubye simvoly — bez etogo bot padayet.
“””
escape_chars = r”[`[]()~>#+-=|{}.!]”
...
“””
Bezopasnaya obrabotka otveta Claude dlya Telegram Markdown.
Ubirayem vse Markdown-konstruktsii, kotorye Claude mog dobavit.
“””
# Ubirayem ** bold ** -> prosto tekst
text = re.sub(r”**(.*?)**”, r”\1”, text)
# Ubirayem * italic * -> prosto tekst
text = re.sub(r”*(.*?)*”, r”\1”, text)
# Ubirayem `code blocks`
text = re.sub(r”`.*?`”, “”, text, flags=re.DOTALL)
# Ubirayem `inline code`
text = re.sub(r”`(.*?)`”, r”\1”, text)
# Ubirayem [links](url)
text = re.sub(r”[(.*?)](.*?)”, r”\1”, text)
# Ekraniruyem ostavshiyesya opasnyye simvoly dlya Markdown V1
text = text.replace(”*”, “\*”)
return text

# ———————————————––

# Generatory demo-dannykh (zaglushki)

# TODO: Zamenit na CoinGecko API / Binance API

# ———————————————––

def _fake_price_data(coin: str) -> dict:
“”“Generatsiya demo-dannykh tseny. ZAGLUSHKA — ne realnyye dannyye!”””
base_prices = {
...
“”“Generatsiya demo-dannykh nastroyeniy. ZAGLUSHKA — ne realnyye dannyye!”””
...
class PriceAgent:
“”“Agent tekhnicheskogo analiza tseny.”””
...
SYSTEM_PROMPT = (
    ...
    "NE ispolzuy Markdown-formatirovaniye (zvyozdochki, podchyorkivaniya i t.d.). "
    "Pishi prostym tekstom."
)
...
async def analyze(self, coin: str) -> dict:
    ...
    try:
        ...
        summary = f"Taym-aut analiza {coin}. Poprobuyte pozhe."
...
“”“Agent analiza nastroyeniy rynka.”””
...
async def analyze(self, coin: str) -> dict:
    ...
    try:
        ...
        summary = f"Taym-aut analiza nastroyeniy {coin}."
    except Exception as e:
...
async def fear_greed_only(self) -> str:
    """Otdelnyy zapros tolko dlya Fear & Greed indeksa."""
    ...
    return (
        ...
        f"*{fg}/100 — {label}*\n\n"
        f"{explanation}\n\n"
        f"_Demo-dannyye (sluchaynyye)_"
    )
```

class OrchestratorAgent:
“”“Glavnyy agent — sinteziruyet dannyye i dayot rekomendatsiyu.”””
